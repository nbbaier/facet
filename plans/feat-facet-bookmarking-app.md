...
- [ ] Create bookmark Server Actions
  ...
  export async function createBookmark(formData: FormData) {
    ...
    if (existing.rows.length > 0) {
      ...
      return { duplicate: true, id: existing.rows[0].id, saveCount: (existing.rows[0].save_count as number) + 1 }
    }
...
- [ ] Create content extraction function
  ```typescript
  // lib/extract.ts
  export async function extractContent(url: string): Promise<{ title: string; content: string; sourceType: string }> {
...
*Simplified plan generated: December 31, 2025*
